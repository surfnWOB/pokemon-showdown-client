...
# Bake in the Spaceworld '97 (gen2sw97) beta sprites. These 36 cut demo species
# have no art on the upstream CDN, so their front/back/shiny PNGs are committed
# under custom-sprites/ (regenerated by build-tools/sw97-sprites.js) and copied
# into the served sprites tree. nginx serves sprites/ local-first, falling back
# to the upstream CDN for everything else (docker/nginx-client.conf).
RUN mkdir -p play.pokemonshowdown.com/sprites \
    && cp -r custom-sprites/gen2sw97/. play.pokemonshowdown.com/sprites/
