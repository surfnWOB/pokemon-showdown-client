...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'rs' | 'rslc' | 'frlg' | 'bw1' | 'letsgo' | 'metronome' |
		...
		'svdlc1natdex' | 'stadium' | 'lc' | 'champions' | 'zangouse' | 'gen3mega' | 'gen3ubersuu' |
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | Dex.PokemonSet = '' as ID) {
		...
		if (this.dex.gen === 3 && (format === 'su' || format === 'iu')) {
			// vanilla gen3 dex; the gen3subzu builder table carries the SU/IU sub-ZU
			// ladder tiers that stock gen3 (correctly) no longer does.
			this.formatType = 'gen3subzu';
		}
