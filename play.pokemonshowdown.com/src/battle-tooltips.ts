...
import { Move, BattleNatures, type Species, type StatName } from "./battle-dex-data";
import { BattleTextParser } from "./battle-text-parser";

type RandbatsDistribution = Record<string, number>;
type RandbatsStatsTable = Partial<Record<StatName, number>>;

interface RandbatsRoleData {
	weight: number;
	abilities?: RandbatsDistribution;
	items?: RandbatsDistribution;
	moves: RandbatsDistribution;
	teraTypes?: RandbatsDistribution;
	evs?: RandbatsStatsTable;
	ivs?: RandbatsStatsTable;
}

interface RandbatsPokemonData {
	name: string;
	level: number;
	abilities?: RandbatsDistribution;
	items?: RandbatsDistribution;
	moves?: RandbatsDistribution;
	roles?: Record<string, RandbatsRoleData>;
	evs?: RandbatsStatsTable;
	ivs?: RandbatsStatsTable;
}

type RandbatsLevelData = Record<ID, RandbatsPokemonData[]>;
type RandbatsFormatData = Record<number, RandbatsLevelData>;

/**
 * Additional possible-set information for Random Battle tooltips.
 *
 * Adapted from pkmn/randbats' MIT-licensed browser extension:
 * https://github.com/pkmn/randbats/blob/main/extension/index.js
 * See battle-tooltips-randbats.LICENSE.
 *
 * Standard formats continue to use pkmn's live feed. Our fork-only
 * `gen3megarandombattle` data is generated by the server and copied into the
 * self-hosted client at build time.
 */
export class RandomBattleTooltip {
	static readonly supportedFormats = [
		...
		'gen3randombattle', 'gen3megarandombattle', 'gen2randombattle',
		'gen1randombattle',
	];
	static readonly localFormats = ['gen3megarandombattle'];
	static readonly data: Record<string, RandbatsFormatData | undefined> = {};
	static readonly requested: Record<string, boolean> = {};

	static resolveFormat(battle: Battle): string {
		let format = toID(battle.tier);
		if (!format?.includes('random')) return '';

		const gen = Number(format.charAt(3));
		if (!Number.isInteger(gen) || gen < 1) return '';

		if (!['singles', 'doubles'].includes(battle.gameType)) {
			format = `gen${gen}randomdoublesbattle` as ID;
		} else if (format.includes('monotype') || format.includes('unrated')) {
			format = `gen${gen}randombattle` as ID;
	...
	static dataURL(format: string): string {
		if (this.localFormats.includes(format)) {
			return `${Dex.resourcePrefix}randbats/data/stats/${format}.json`;
		}
		return `https://pkmn.github.io/randbats/data/stats/${format}.json`;
	}

	static load(battle: Battle) {
		const format = this.resolveFormat(battle);
		if (!format || this.data[format] || this.requested[format]) return;
		this.requested[format] = true;

		const request = new XMLHttpRequest();
		request.addEventListener('load', () => {
			try {
				const json = JSON.parse(request.responseText) as Record<string, Omit<RandbatsPokemonData, 'name'>>;
				const data: RandbatsFormatData = {};
				const dex = Dex.forFormat(format);

				for (const name in json) {
					const pokemon = json[name];
					const level = name.startsWith('Zoroark') ? 0 : pokemon.level;

					const levelData = data[level] ||= {};
					const species = dex.species.get(name);
					if (!species.exists) continue;

					const battleOnly = typeof species.battleOnly === 'string' ?
						species.battleOnly : species.name;
					const lookupName = species.forme === 'Gmax' ? species.baseSpecies :
						battleOnly || species.name;
					const id = toID(lookupName);
					const sets = levelData[id] ||= [];
					sets.push({ ...pokemon, name, level });
				}

				this.data[format] = data;
			} catch (error) {
				console.error(
					`Unable to load Random Battle tooltip data for ${format}.`,
					error
				);
			}
		});
		request.open('GET', this.dataURL(format));
		request.send(null);
	}

	static render(battle: Battle, clientPokemon: Pokemon): string {
		if (battle.hardcoreMode) return '';

		const format = this.resolveFormat(battle);
		if (!format) return '';
		this.load(battle);

		const formatData = this.data[format];
		...
		const levelData = formatData[species.baseSpecies === 'Zoroark' ? 0 : clientPokemon.level];
		if (!levelData) return '';

		const cosmetic = !!species.cosmeticFormes?.includes(species.name);
		const battleOnly = typeof species.battleOnly === 'string' ?
			species.battleOnly : species.name;
		let id = toID(
			(species.forme === 'Gmax' || cosmetic) ?
				species.baseSpecies :
				battleOnly || species.name
		);
		if (id.startsWith('pikachu')) id = id.endsWith('gmax') ? 'pikachugmax' as ID : 'pikachu' as ID;

		let forme = cosmetic ? species.baseSpecies : clientPokemon.speciesForme;
		if (forme.startsWith('Pikachu')) forme = forme.endsWith('Gmax') ? 'Pikachu-Gmax' : 'Pikachu';

		let sets = levelData[id];
		if (!sets) return '';
		const greninjaBondSets = levelData['greninjabond' as ID];
		if (id === 'greninja' && greninjaBondSets) {
			sets = sets.concat(greninjaBondSets);
		}

		if (sets.length === 1) {
			return this.renderSet(
				gen, battle.gameType, letsgo, species,
				{ ...sets[0], level: clientPokemon.level }, '', clientPokemon
			);
		}

		if (toID(forme) !== id) {
			const matches = sets.filter(set => set.name === forme);
			if (matches.length === 1) {
				return this.renderSet(
					gen, battle.gameType, letsgo, species,
					{ ...matches[0], level: clientPokemon.level }, '', clientPokemon
				);
			}
		}

		return sets.map(set => this.renderSet(
			gen, battle.gameType, letsgo, species,
			{ ...set, level: clientPokemon.level }, set.name, clientPokemon
		)).join('');
	}
	...
	static renderSet(
		...
		letsgo: boolean,
		species: Species,
		data: RandbatsPokemonData,
		name: string,
		clientPokemon: Pokemon
	): string {
		const noHiddenPower = !Object.keys(data.moves || {}).some(move => move.startsWith('Hidden Power'));
		let buf = '<div style="border-top:1px solid #888;background:#dedede">';
		if (name) buf += `<p><strong>${BattleLog.escapeHTML(name)}</strong></p>`;

		const multi = !['singles', 'doubles'].includes(gameType);
		if (data.roles) {
			const roles = this.filterRoles(data.roles, clientPokemon);
			if (!roles.length) return '';

			for (let i = 0; i < roles.length; i++) {
				const [roleName, role] = roles[i];
				buf += i ? '<div style="border-top:1px solid #888">' : '<div>';
				buf += `<p><u>${BattleLog.escapeHTML(roleName)}</u> ` +
					`<small>(${Math.round(role.weight * 100)}%)</small></p>`;
				if (gen >= 3 && !letsgo) {
					buf += `<p><small>Abilities:</small> ${this.display(role.abilities)}</p>`;
				}
				if (gen >= 2 && !(letsgo && !role.items)) {
					buf += `<p><small>Items:</small> ${
						role.items ? this.display(role.items) : '(No Item)'
					}</p>`;
				}
				if (gen === 9 && role.teraTypes) {
					buf += `<p><small>Tera Types:</small> ${this.display(role.teraTypes)}</p>`;
				}
				buf += `<p><small>Moves:</small> ${this.display(role.moves, multi)}</p>`;
				buf += this.renderStats(gen, letsgo, species, role, data.level, noHiddenPower);
				buf += '</div>';
			}
		} else {
			if (gen >= 3 && !letsgo) {
				buf += `<p><small>Abilities:</small> ${this.display(data.abilities)}</p>`;
			}
			if (gen >= 2 && !(letsgo && !data.items)) {
				buf += `<p><small>Items:</small> ${
					data.items ? this.display(data.items) : '(No Item)'
				}</p>`;
			}
			buf += `<p><small>Moves:</small> ${this.display(data.moves, multi)}</p>`;
			buf += this.renderStats(gen, letsgo, species, data, data.level, noHiddenPower);
		}
	...
	static renderStats(
		...
		letsgo: boolean,
		species: Species,
		data: { evs?: RandbatsStatsTable, ivs?: RandbatsStatsTable },
		level: number,
		noHiddenPower: boolean
	): string {
		const stats = {} as Record<StatName, number>;
		for (const stat of Dex.statNames) {
			stats[stat] = this.calcStat(
				gen,
				stat,
				species.baseStats[stat],
				data.ivs?.[stat] ?? (gen < 3 ? 30 : 31),
				data.evs?.[stat] ?? (gen < 3 ? 255 : letsgo ? 0 : 85),
				level,
				letsgo
			);
		}
		...
		for (const statName of Dex.statNamesExceptHP) {
			...
			const known = gen === 1 || (gen === 2 && noHiddenPower) ||
	...
	static filterRoles(
		roles: Record<string, RandbatsRoleData>,
		clientPokemon: Pokemon
	): [string, RandbatsRoleData][] {
		return Object.entries(roles).filter(([, role]) => {
			if (clientPokemon.terastallized && !role.teraTypes?.[clientPokemon.terastallized]) return false;
			return clientPokemon.moveTrack.every(([moveName]) => (
				!!role.moves[moveName] ||
				(moveName === 'Hidden Power' && Object.keys(role.moves).some(move => move.startsWith('Hidden Power')))
			));
		});
	}

	static display(distribution?: RandbatsDistribution, multi = false): string {
		if (!distribution) return '';
		const values = [];
		for (const name in distribution) {
			const frequency = distribution[name];
			if (!frequency || (multi && name === 'Ally Switch')) continue;
			values.push(
				BattleLog.escapeHTML(name) +
				(frequency >= 1 ? '' : ` <small>(${Math.round(frequency * 100)}%)</small>`)
			);
		}
		return values.join(', ');
	}
	...
	static calcStat(
		...
		if (stat === 'hp') {
			const value = base === 1 ? base :
				Math.trunc(Math.trunc(2 * base + iv + Math.trunc(ev / 4) + 100) * level / 100 + 10);
			return letsgo ? value + 20 : value;
		}
		const value = Math.trunc(Math.trunc(2 * base + iv + Math.trunc(ev / 4)) * level / 100 + 5);
		return letsgo ? Math.trunc(value * 102 / 100) + 20 : value;
	}
}
...
export class BattleTooltips {
	...
	loadRandomBattleData() {
		RandomBattleTooltip.load(this.battle);
	}
	...
	showPokemonTooltip(
		...
		if (!this.battle.hardcoreMode && clientPokemon && !serverPokemon) {
			text += RandomBattleTooltip.render(this.battle, clientPokemon);
		}
		return text;
	}
...
if (typeof require === 'function') {
	...
	global.RandomBattleTooltip = RandomBattleTooltip;
}
